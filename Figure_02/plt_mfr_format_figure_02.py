#!/usr/bin/env python3
"""
Mean Firing Rate heatmap — formatted to match Figure 02 layout.

Input : all_metrics.csv  (columns: noise, coupling, mean_firing_rate, ...)
Output: formatted_mfr_metric.png
"""
...
OUTPUT_FILE = "formatted_mfr_metric.png"
METRIC_COL  = "mean_firing_rate"
...
# Circle label style — same as Figure 02
LABEL_SIZE = 11.0
LABEL_XOFF = 0.60
LABEL_YOFF = 0.20
LABEL_PAD  = 0.25
...
def plot_mfr_heatmap(df):
    col_map = {c.lower().strip(): c for c in df.columns}
    ncol = col_map["noise"]
    ccol = col_map["coupling"]
    mcol = col_map[METRIC_COL]
    ...
    # Colorbar — continuous MFR scale
    cbar = hm.collections[0].colorbar
    cbar.set_label(r"$\mathrm{MFR}$", fontsize=16)
...
def main():
    ...
    fig, ax = plot_mfr_heatmap(df)

    plt.tight_layout()

    # Shift Y-label up 3 mm (post-layout adjustment matching Figure 02)
    dy_in = 3 / 25.4
    offset = mtransforms.ScaledTranslation(0, dy_in, fig.dpi_scale_trans)
    ax.yaxis.get_label().set_transform(
        ax.yaxis.get_label().get_transform() + offset
