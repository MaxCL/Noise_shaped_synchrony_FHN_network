#!/usr/bin/env python3
"""
Panel (b) — normalized vectors for a single input time t.
Usage: python3 plt_panel_b_normalized_t_.py <t>
  <t>  time in thousands of steps, e.g. 412.01
Output: panel_b_normalized_<tttttt>_.png  (6-digit integer = round(t*1000))
"""
...
def plot_panel(ax, u_tup, v_tup, t, cfg, is_bottom=False):
    ...
    # inset boundaries — fixed to PANEL_B t0/t1, unchanged by t
    ia_abs  = to_abs(t0)
    ib_abs  = to_abs(t1)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 plt_panel_b_normalized_t_.py <t>")
        print("  <t>  time in thousands of steps, e.g. 412.01")
        sys.exit(1)

    t     = float(sys.argv[1])
    t_str = f"{int(round(t * 1000)):06d}"
    out   = f"panel_b_normalized_{t_str}_.png"

    uB = load_csv(U_CSV_B)
    vB = load_csv(V_CSV_B)

    fig, ax = plt.subplots(1, 1, figsize=(6.5, 3.75), dpi=300)
    plot_panel(ax, uB, vB, t, PANEL_B, is_bottom=True)
    plt.tight_layout()
