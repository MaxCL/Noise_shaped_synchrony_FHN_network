#!/usr/bin/env python3
"""
Figure 01 — raw (unnormalised) vectors variant of plt_f1_git.py.
Vectors are scaled by _SCALE instead of being normalised to unit length.
Output: figure_01_raw_vectors.png
"""
...
# ── Data files (CSV subsets extracted from the full binary runs) ─────────────
...
# ── Panel configurations (canonical values from the notebook) ────────────────
...
# ── FHN / style constants (from repo figures_lib.py) ────────────────────────
_A     = 0.99
_I     = 0.0
_P     = 10
_UMIN, _UMAX = -2.5, 2.0
...
# ── Helpers inlined from repo figures_lib.py ─────────────────────────────────
...
def plot_panel(ax, u_tup, v_tup, cfg, is_bottom=False):
    ...
    ax.scatter(u_mat[ia, :][mask], v_mat[ia, :][mask], s=16, c="blue",  alpha=0.5, zorder=4)
    ax.scatter(u_mat[ib, :][mask], v_mat[ib, :][mask], s=16, c="green", alpha=0.5, zorder=4)

    u0, v0 = float(u_mat[ia, node]), float(v_mat[ia, node])
    u1, v1 = float(u_mat[ib, node]), float(v_mat[ib, node])
    ax.scatter([u0], [v0], s=40, c="red", alpha=0.9, zorder=5)
    ax.scatter([u1], [v1], s=40, c="red", alpha=0.9, zorder=5)
    ax.text(u0 + 0.04, v0 - 0.16, r"$t_0$", alpha=0.9, color="blue",  fontsize=18)
    ax.text(u1 + 0.04, v1 - 0.21, r"$t_1$", alpha=0.9, color="green", fontsize=18)

    Cu0 = float(Cu_at(u_mat, node, ia)); Cv0 = float(Cv_at(v_mat, node, ia))
    Cu1 = float(Cu_at(u_mat, node, ib)); Cv1 = float(Cv_at(v_mat, node, ib))
    Lu0 = u0 - (u0**3)/3.0 - v0;  Lv0 = u0 + _A
    Lu1 = u1 - (u1**3)/3.0 - v1;  Lv1 = u1 + _A
    s0 = noise_at(node, int(u_idx[ia]))
    s1 = noise_at(node, int(u_idx[ib]))

    Cs0 = np.array([Cu0, Cv0]) * _SCALE
    Ls0 = np.array([Lu0, Lv0]) * _SCALE
    Ns0 = np.array([0.0, s0])  * _SCALE
    Cs1 = np.array([Cu1, Cv1]) * _SCALE
    Ls1 = np.array([Lu1, Lv1]) * _SCALE
    Ns1 = np.array([0.0, s1])  * _SCALE
    plot_node_vectors(ax, u0, v0, [Cs0, Ls0, Ns0], ["hotpink", "blueviolet", "teal"])
...
# ── Main ─────────────────────────────────────────────────────────────────────
def main():
    uA = load_csv(U_CSV_A); vA = load_csv(V_CSV_A)
    uB = load_csv(U_CSV_B); vB = load_csv(V_CSV_B)

    fig, axes = plt.subplots(2, 1, figsize=(6.5, 7.5), dpi=300, sharex=False)
    plot_panel(axes[0], uA, vA, PANEL_A, is_bottom=False)
    plot_panel(axes[1], uB, vB, PANEL_B, is_bottom=True)
    plt.tight_layout()
    plt.savefig("figure_01_raw_vectors.png", bbox_inches="tight", dpi=700)
    print("Saved figure_01_raw_vectors.png")
