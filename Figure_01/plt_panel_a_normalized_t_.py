#!/usr/bin/env python3
"""
Panel (a) — normalized vectors for a single input time t.
Usage: python3 plt_panel_a_normalized_t_.py <t>
  <t>  time in thousands of steps, e.g. 269.265
Output: panel_a_normalized_<tttttt>_.png  (6-digit integer = round(t*1000))
"""
...
# ── Data files ────────────────────────────────────────────────────────────────
U_CSV_A  = "/tmp/fhn_repo/Figure_01/u_n_00060_c_00600_.csv"
V_CSV_A  = "/tmp/fhn_repo/Figure_01/v_n_00060_c_00600_.csv"
...
PANEL_A = dict(node=29, t0=259.11, t1=269.265, dt0=2.0, dt1=2.0,
               title=r"(a)", show_nullclines=True, show_limit_cycle=True, show_vectors=True)
...
def add_spacetime_inset(ax, u_mat, ia, ib, node, ia_core, ib_core, it=None):
    ...
    ins.add_patch(rect)
    if it is not None:
        ins.axvline(x=it, color="#39FF14", lw=2.0, alpha=0.4, zorder=7)
...
def plot_panel(ax, u_tup, v_tup, t, cfg, is_bottom=False):
    node, t0, t1, dt0, dt1 = cfg["node"], cfg["t0"], cfg["t1"], cfg["dt0"], cfg["dt1"]
    u_idx, u_mat = u_tup
    v_idx, v_mat = v_tup

    # inset boundaries — fixed to PANEL_A t0/t1, unchanged by t
    ia_abs  = to_abs(t0)
    ib_abs  = to_abs(t1)
    ...
    # main panel row — determined by input t
    it = row_of(u_idx, to_abs(t))
    ...
    ax.scatter(u_mat[it, :][mask], v_mat[it, :][mask], s=16, c="green", alpha=0.5, zorder=4)

    ut, vt = float(u_mat[it, node]), float(v_mat[it, node])
    ax.scatter([ut], [vt], s=40, c="red", alpha=0.9, zorder=5)

    Cut = float(Cu_at(u_mat, node, it)); Cvt = float(Cv_at(v_mat, node, it))
    Lut = ut - (ut**3)/3.0 - vt;  Lvt = ut + _A
    st  = noise_at(node, int(u_idx[it]))

    Cst = unit([Cut, Cvt]) * _L;  Lst = unit([Lut, Lvt]) * _L;  Nst = unit([0.0, st]) * _L
    plot_node_vectors(ax, ut, vt, [Cst, Lst, Nst], ["hotpink", "blueviolet", "teal"])

    add_spacetime_inset(ax, u_mat, iai, ibi, node, ia_core=ia, ib_core=ib, it=it)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 plt_panel_a_normalized_t_.py <t>")
        print("  <t>  time in thousands of steps, e.g. 269.265")
        sys.exit(1)

    t     = float(sys.argv[1])
    t_str = f"{int(round(t * 1000)):06d}"
    out   = f"panel_a_normalized_{t_str}_.png"

    uA = load_csv(U_CSV_A)
    vA = load_csv(V_CSV_A)

    fig, ax = plt.subplots(1, 1, figsize=(6.5, 3.75), dpi=300)
    plot_panel(ax, uA, vA, t, PANEL_A, is_bottom=True)
    plt.tight_layout()
    plt.savefig(out, bbox_inches="tight", dpi=700)
    print(f"Saved {out}")
